...
public class MenuImpl implements Menu {
  ...
  public MenuItem menuItem(final String displayName) throws CCAPIException {
    ...
    try {
      new WebDriverWait(this.webDriver, Constants.WEBDRIVER_WAIT)
          .until((ExpectedCondition<Boolean>) d -> {
            final List<WebElement> li = Utils.findMultipleItemsInElement(MenuImpl.this.webDriver,
                menuElement, By.tagName("li"));
    ...
    final List<WebElement> items =
        Utils.findMultipleItemsInElement(this.webDriver, menuElement, By.tagName("li"));
    final WebElement item =
        items.stream().filter(i -> i.getText().equals(displayName)).findFirst().orElse(null);

    if (item != null) {
      return new MenuItemImpl(this, item.getText(), this.webDriver);
    } else {
      throw new CCAPIException("Can't find menu item with the displayname: '" + displayName + "'",
          this.webDriver);
    }
  }
  ...
  public List<MenuItem> menuItems() throws CCAPIException {
    final WebElement menuElement = this.open();
    final List<WebElement> menuWebElements =
        Utils.findMultipleItemsInElement(this.webDriver, menuElement, By.tagName("li"));
  ...
  public WebElement open() throws CCAPIException {
    final WebElement button =
        Utils.find(this.webDriver, ExpectedConditions.elementToBeClickable(this.selector));
    ((JavascriptExecutor) this.webDriver).executeScript(MenuImpl.JS_MOUSEOVER, button);
    return Utils.find(this.webDriver, ExpectedConditions
        .presenceOfElementLocated(By.cssSelector("body > div.fs-toolbar-flyout")));
  }
