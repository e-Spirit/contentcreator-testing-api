...
public abstract class AbstractReport implements Report {
    ...
    AbstractReport(@NotNull final WebDriver webDriver, @NotNull final WebElement reportButton) {
    ...
    public WebElement getEntry(final int pos) throws CCAPIException {
        if (this.isVisible()) {
            return Utils.findElement(this.webDriver, By.cssSelector("div.report-entry-container > div > div > div:nth-child(1) > div:nth-child(" + (pos + 1) + ") > div"));
        }
    ...
    @Override
    public void toggle() {
        final WebElement button = Utils.find(this.webDriver, ExpectedConditions.elementToBeClickable(this.reportButton));
        button.click();
    }
    ...
    public List<CCInputComponent> parameters() throws CCAPIException {
        ...
        final WebElement reportContent = Utils.find(this.webDriver, ExpectedConditions.visibilityOfElementLocated(By.className("fs-sidebar-content")));
        final WebElement parametersDiv = Utils.findItemInElement(this.webDriver, reportContent, By.className("fs-sidebar-report-parameter"));
        final List<WebElement> elements = Utils.findMultipleItemsInElement(this.webDriver, parametersDiv, By.cssSelector("div > table > tbody > tr"));
        final List<CCInputComponent> result = new ArrayList<>();

        for (final WebElement element : elements) {
            final CCInputComponent ccInputComponent = ComponentUtils.matchParameter(element, this.webDriver);
