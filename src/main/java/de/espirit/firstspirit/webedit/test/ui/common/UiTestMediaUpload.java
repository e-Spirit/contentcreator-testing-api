...
import de.espirit.firstspirit.webedit.test.ui.RemoteIEWebDriverFactory;
...
public class UiTestMediaUpload extends AbstractUiTest {
	...
	public void fileUpload() throws Exception {
		...
		((JavascriptExecutor) we().driver()).executeScript(
				FileUtilities.inputStreamToString(UiTestInlineEdit.class.getResourceAsStream("FireEvent.js"), "UTF-8"),
				"click", upload
		); // in IE11 a simple "upload.click()" causes an exception because the flyout is closed before the event is fired
		...
		((JavascriptExecutor) we().driver()).executeScript(
				FileUtilities.inputStreamToString(UiTestMediaUpload.class.getResourceAsStream("UiTestMediaUpload.js"), "UTF-8"),
				dropTarget,
				uid
		);
